...
const main = async () => {
    ...
    // const item = await colores.doc('2lUnhaqwRa4V9Un9pE0A').update({ color: 'orange' });
    // console.log("El color ha sido actualizado", item);
    
    
    // /* --------------------------------------- */
    // const itemDeleted = await colores.doc('URG4ssFDQXInXbSuepmo').delete();
    // console.log("El color ha sido borrado exitosamente", itemDeleted);

    const snapshotFinal = await colores.get();
    snapshotFinal.docs.forEach(doc => {
        console.log({ id: doc.id, ...doc.data() })
    })
};
